...
def commit_update(symList):
    ...
    try:
            cursor.execute(f"INSERT INTO {table_name} (symList) VALUES (:1)", (symList,))
            connection.commit()
            print("Data inserted successfully!")
            print(f"ok")
    except oracledb.DatabaseError as e :
        if "table or view does not exist" in str(e):
            print(f"table \"{table_name}\" does not exist") # ORA-00942: table or view does not exist
            # create new table col ut symlist 
            cursor.execute(f"CREATE TABLE {table_name} (ut TIMESTAMP DEFAULT CURRENT_TIMESTAMP, symList VARCHAR2(255),PRIMARY KEY (ut))")
            cursor.execute(f"INSERT INTO {table_name} (symList) VALUES (:1)", (symList,))
            connection.commit()
...
async def dump_yaml(cc, config):
    ...
    print(f"Updating sorted_syms ...")
    commit_update(str(sorted_syms))
    send_msg("Commited!")
...
# ---------------------------------------------------------------------------
            if args.no_loop:
                return
            for i in range((config["update_interval_minutes"]-30) * 60, -1, -1):
                time.sleep(1)
                print(f"\rcountdown: {i}    ", end=" ")
            print()
        except Exception as e:
